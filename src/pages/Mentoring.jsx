...
function Mentoring() {
  ...
  const ReadDatabase = () => {
    axios.get("http://localhost:8083/dashboardmentor").then((res) => {
      ...
      <main className="dark:bg-gray-800 bg-white">
        ...
        <div className="container mx-auto flex flex-col md:flex-row flex-wrap justify-start">
          {dashboardmentor.map((data, id) => (
            <div key={id} className="lg:w-96 md:w-1/3 w-full p-4 ">
              <div className="p-8 rounded-xl border-2 bg-gray-50 hover:shadow-lg transition-transform transition-ease-in-out hover:transform hover:scale-[102%] border-gray-200">
                <h4 className="mb-2 text-lg font-semibold mt-4">{data.namamentor}</h4>
                <p className="text-base">{batasankata(data.deskripsimentor, 15)}</p>
                <h3 className="mt-2 text-sm font-medium text-blue-400">{data.minatpenelitian}</h3>

                <p className="mt-1.5 text-sm text-gray-500">{data.fee}</p>

                <form className="mt-4 flex flex-col md:flex-row gap-2">
                  <Link to={`/booking`}>
                    <button className="text-white w-full md:w-1/2 md:ml-1 rounded bg-red-400 p-4 text-sm font-semibold transition hover:scale-105">Booking</button>
                  </Link>
                  <Link to={`/detailmentor/${data.id}`}>
                    <button className="text-white block w-full rounded bg-blue-700 p-4 text-sm font-semibold transition ease-in-out hover:scale-105">Details</button>
                  </Link>
                </form>
              </div>
            </div>
